...
class YoloDetectionResponse {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.detected_objects[i] = Detection.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.detected_objects !== undefined) {
      ...
      for (let i = 0; i < resolved.detected_objects.length; ++i) {
        resolved.detected_objects[i] = Detection.Resolve(msg.detected_objects[i]);
      }
    }
    else {
...
module.exports = {
  ...
  datatype() { return 'lasr_vision_msgs/YoloDetection'; }
};
